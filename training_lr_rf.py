...
def plot_metrics_comparison(metrics_lr, metrics_rf, path):
    metrics = ['Accuracy', 'Precision', 'Recall', 'F1_Score']
    lr_vals = [metrics_lr[m] for m in metrics]
    rf_vals = [metrics_rf[m] for m in metrics]
    x = np.arange(len(metrics))
    plt.figure(figsize=(8, 6))
    plt.bar(x-0.15, lr_vals, width=0.3, label='Logistic Regression', color='blue')
    plt.bar(x+0.15, rf_vals, width=0.3, label='Random Forest', color='green')
...
def run_training(
    ...
    if use_pca and n_pca_components_list:
        for n_components in n_pca_components_list:
            print(f"\nRunning with PCA, n_components={n_components}")
            pca_result_base = os.path.join(result_base, f"pca{n_components}")
            os.makedirs(pca_result_base, exist_ok=True)
            
            # Generate feature names for principal components without transforming data yet
            num_features = min(n_components, X.shape[1])
            feature_names = [f'PC{i+1}' for i in range(num_features)]
            
            # The pipeline will handle scaling and PCA fitting on the training data
            run_test_split_and_models(df, X, y, label_encoder, feature_names, pca_result_base, test_df, feature_cols, use_pca=True, n_pca_components=n_components)
    else:
        os.makedirs(result_base, exist_ok=True)
        run_test_split_and_models(df, X, y, label_encoder, feature_cols, result_base, test_df, feature_cols, use_pca=False)

def run_test_split_and_models(df, X, y, label_encoder, feature_names, result_base, test_df, original_feature_cols, use_pca=False, n_pca_components=None):
    ...
    for test_name in test_names:
        if test_name == 'all':
            ...
            # This part seems to have a logic issue for splitting, as it would retrain on the whole dataset.
            # Re-implementing to split based on 'test' column.
        ...
        def build_pipeline(model, use_pca, n_pca_components):
            ...
            if use_pca:
                # Ensure n_components is not greater than number of features
        ...
        if HYPERPARAMETER_TUNING:
            lr_grid = GridSearchCV(lr_pipe, param_grid_lr, cv=3, n_jobs=4, return_train_score=True)
            lr_grid.fit(X_train, y_train)
            lr = lr_grid.best_estimator_
            pd.DataFrame([lr_grid.best_params_]).to_csv(os.path.join(test_dir, "lr_best_params.csv"), index=False)
            pd.DataFrame(lr_grid.cv_results_).to_csv(os.path.join(test_dir, "lr_all_cv_results.csv"), index=False)
        else:
        ...
        plot_confusion_matrix(cm_lr, labels, "Logistic Regression Confusion Matrix", os.path.join(test_dir, "lr_confusion_matrix.png"))
        plot_roc_curve(y_val, y_proba_lr, labels, os.path.join(test_dir, "lr_roc_curve.png"), "Logistic Regression ROC Curve")
        
        if use_pca:
            # When using PCA, coefficients are for principal components
            lr_coef = lr.named_steps['classifier'].coef_
        else:
            lr_coef = lr.named_steps['classifier'].coef_
        plot_feature_importance(np.mean(np.abs(lr_coef), axis=0), feature_names, test_dir, "Logistic Regression Feature Importance")
        pd.DataFrame(cm_lr, index=labels, columns=labels).to_csv(os.path.join(test_dir, "lr_confusion_matrix.csv"))

        # --- Random Forest ---
        print(f"Training Random Forest for test: {test_name}")
        ...
        if HYPERPARAMETER_TUNING:
            rf_grid = GridSearchCV(rf_pipe, param_grid_rf, cv=3, n_jobs=4, return_train_score=True)
            rf_grid.fit(X_train, y_train)
            rf = rf_grid.best_estimator_
            pd.DataFrame([rf_grid.best_params_]).to_csv(os.path.join(test_dir, "rf_best_params.csv"), index=False)
            pd.DataFrame(rf_grid.cv_results_).to_csv(os.path.join(test_dir, "rf_all_cv_results.csv"), index=False)
        else:
        ...
        plot_confusion_matrix(cm_rf, labels, "Random Forest Confusion Matrix", os.path.join(test_dir, "rf_confusion_matrix.png"))
        plot_roc_curve(y_val, y_proba_rf, labels, os.path.join(test_dir, "rf_roc_curve.png"), "Random Forest ROC Curve")
        
        if not use_pca:
            importances = rf.named_steps['classifier'].feature_importances_
            plot_feature_importance(importances, feature_names, test_dir, "Random Forest Feature Importance")
        else:
            print("Feature importance for Random Forest with PCA is not directly interpretable from the model and is not plotted.")

        pd.DataFrame(cm_rf, index=labels, columns=labels).to_csv(os.path.join(test_dir, "rf_confusion_matrix.csv"))
        plot_metrics_comparison(metrics_lr, metrics_rf, os.path.join(test_dir, "metrics_comparison.png"))
        print(f"Results for validation set '{test_name}' saved in: {test_dir}")
        ...
        plot_confusion_matrix(cm_lr_test, labels, "Logistic Regression Confusion Matrix (Test Set)", os.path.join(test_dir, "lr_confusion_matrix_test.png"))
        plot_roc_curve(y_test_external, y_proba_lr_test, labels, os.path.join(test_dir, "lr_roc_curve_test.png"), "Logistic Regression ROC Curve (Test Set)")
        pd.DataFrame(cm_lr_test, index=labels, columns=labels).to_csv(os.path.join(test_dir, "lr_confusion_matrix_test.csv"))
        ...
        plot_confusion_matrix(cm_rf_test, labels, "Random Forest Confusion Matrix (Test Set)", os.path.join(test_dir, "rf_confusion_matrix_test.png"))
        plot_roc_curve(y_test_external, y_proba_rf_test, labels, os.path.join(test_dir, "rf_roc_curve_test.png"), "Random Forest ROC Curve (Test Set)")
        pd.DataFrame(cm_rf_test, index=labels, columns=labels).to_csv(os.path.join(test_dir, "rf_confusion_matrix_test.csv"))
        
        print(f"Results for external test set saved in: {test_dir}")

def feature_selection_and_training(feature_csv, analysis_groups, result_folder, train_df, test_df, label_col):
    feature_df = pd.read_csv(feature_csv)
    for group_name, analysis_values in analysis_groups.items():
        print(f"\n=== Running for feature group: {group_name} ===")
        ...
        print(f"Number of selected features after filtering and intersection: {len(feature_cols)}")
        result_base = os.path.join(result_folder, group_name)
        run_training(train_df, test_df, label_col, feature_cols, result_base, USE_PCA, N_PCA_COMPONENTS_LIST if USE_PCA else None)
...
if __name__ == "__main__":
    ...
    plot_class_distribution(train_df, test_df, label_col, "result")
    ...
    feature_selection_and_training(
        ...
        flc_groups,
        "../result/feature_label_correlation",
        train_df,
        test_df,
        label_col
    )
